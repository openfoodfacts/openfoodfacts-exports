...
def process_lang(tags: list[str], lang: str) -> str:
    ...
    - [en:glass, en:bottle]
    """
    return [tag.split(":")[-1] for tag in tags if tag.startswith(f"{lang}:")]


def process_text(texts: list[dict], lang: str) -> str | None:
    """In the Parquet, texts are grouped by lang as a dict as such{"lang": "pl", "text": <text>}"""
    for elt in texts:
        return elt["text"] if elt["lang"] == lang else ""
